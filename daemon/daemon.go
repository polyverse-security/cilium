...
func NewDaemon(c *Config) (*Daemon, error) {
	...
	case kvstore.Consul:
		if c.ConsulConfig != nil {
			...
			log.Warningf("Invalid configuration for consul provided; using local storage.")
	...
	case kvstore.Etcd:
		if c.EtcdCfgPath != "" || c.EtcdConfig != nil {
			...
			log.Warningf("Invalid configuration for etcd provided; using local storage.")
			kvClient = kvstore.NewLocalClient()
		}
	default:
		log.Infof("Using local storage")
		kvClient = kvstore.NewLocalClient()
	}
